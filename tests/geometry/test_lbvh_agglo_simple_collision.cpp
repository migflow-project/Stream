    ...
    ava_for<256>(nullptr, 0, n, [=] __device__ (int const tid) {
        ...
        while (stack_size != 0) {
            ...
            for (int ichild = 0; ichild < 2; ichild++){
                uint32_t child_id = children[ichild];
                bool const is_leaf = child_id < n_v;

                // NOTE: We can also check directly the ranges of internal nodes
                // uint32_t range_min = d_range_min_v(child_id);
                // uint32_t range_max = d_range_max_v(child_id);
                // Child is an internal node
                // if (range_max - range_min < LEAF_SIZE) { 
                if (!is_leaf) {
                    ...
                    const Sphere2D obj = d_obj_m_v(child_id);
                    float const rad_sum = query.r + obj.r;
                    float const d2 = (query.c - obj.c).sqnorm();
                    if (d2 <= rad_sum*rad_sum && (tid != child_id)) ncoll_loc++;
                }
            }
        }
