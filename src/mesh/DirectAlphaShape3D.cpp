...
#define N_BND 128
    ...
    ERRCHK(ava::scan::inclusive_sum(nullptr, temp_mem_size, d_node_nelem_out->data, d_elemrow->data + 1, n_nodes));
    d_temp_mem->resize({temp_mem_size});

    ERRCHK(ava::scan::inclusive_sum(d_temp_mem->data, temp_mem_size, d_node_nelem_out->data, d_elemrow->data + 1, n_nodes));
    deep_copy(&n_elems, d_elemrow->data + n_nodes, 1);

    ERRCHK(ava::scan::inclusive_sum(d_temp_mem->data, temp_mem_size, d_node_nfneig->data, d_row->data + 1, n_nodes));
